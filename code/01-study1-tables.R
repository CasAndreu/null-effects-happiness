#===============================================================================
# 01-study1-tables.R
# Purpose: To replicate Study 1 tables in the appendix of the paper, where we report the descriptives 
...
dat_numvars <- names(dat)[
  which(!(names(dat) %in% 
            c("condition", "partisanship","gender", "race", "edu")))]
for (v in dat_numvars) {
  dat[,v] = as.numeric(as.character(dat[,v]))
}

dat_facvars = c("condition", "partisanship","gender", "race", "edu")
...
dat = dat %>%
  mutate_at(.funs = scales::rescale, .vars = vars(-duration, -age, -gender, -race, -edu, -attention_check, -condition, -partisanship,-post_tiredness, -post_happiness_semantic, -pre_happiness_semantic, -pre_tiredness))
...
moderators = c("pids", "interest", "partisanship","ideology","panas")
...
out_dd_interest = data.frame(term = c("Intercept", "Happy Photos", "Happy Questions", "Happy Writing","Interest", 
                                      ...
                                      "N","R-squared"))
out_dd_partisanship = data.frame(term = c("Intercept", "Happy Photos", "Happy Questions", "Happy Writing","Partisanship(R)", 
                                          "Happy Photos × Partisanship(R)", "Happy Questions × Partisanship(R)" , "Happy Writing × Partisanship(R)" , 
                                          "N","R-squared"))
out_dd_ideology = data.frame(term = c("Intercept", "Happy Photos", "Happy Questions", "Happy Writing","Ideology",
                                      "Happy Photos × Ideology", "Happy Questions × Ideology" , "Happy Writing × Ideology" , 
                                      "N","R-squared"))
out_dd_panas = data.frame(term = c("Intercept", "Happy Photos", "Happy Questions", "Happy Writing","Happiness",
...
#### fdr adjustment ####
out_dd_all = list(out_dd, out_dd_pids, out_dd_interest, out_dd_partisanship, out_dd_ideology, out_dd_panas)
pvalue_all = NULL
for (i in (1:6)){
  for (j in (seq(4, 46, 3))){
...
for (i in (1:15)){
  ...
  out_dd_panas = out_dd_panas %>%
    add_column(p.fdr = pvalue_all$p.fdr[c((681+10*i):(690+10*i))], 
               .after = 4*i,
               .name_repair = "minimal") %>%
    mutate(!!dv_list[i] := ifelse(is.na(out_dd_panas[ ,3]), out_dd_panas[ ,(4*i-2)], paste0(out_dd_panas[ ,(4*i-2)], replace(pvalue_all$p.sig[c((681+10*i):(690+10*i))], is.na(pvalue_all$p.sig[c((681+10*i):(690+10*i))]),""), " (", out_dd_panas[ ,(4*i-1)], ")"))
    )
}
...
out_dd_panas_final = out_dd_panas[ , c(1, seq(2, 58, 4))] %>%
  rename(
    ` ` = term,
    `Affective polarization` = ap_index,
    `FT immigrants` = ft_immigrant,
...
out_dd_panas_final_main = out_dd_panas_final[ ,c(1:7)]
out_dd_panas_final_indi_diff = out_dd_panas_final[ ,c(1,8:16)]

write.table(out_dd_final_main, file = "study1_maineffecits_main.txt", sep = ",", quote = FALSE, row.names = F, na = "")
write.table(out_dd_final_indi_diff, file = "study1_maineffecits_individual_and_difference.txt", sep = ",", quote = FALSE, row.names = F, na = "")

write.table(out_dd_pids_final_main, file = "study1_moderation_pids_main.txt", sep = ",", quote = FALSE, row.names = F, na = "")
write.table(out_dd_pids_final_indi_diff, file = "study1_moderation_pids_individual_and_difference.txt", sep = ",", quote = FALSE, row.names = F, na = "")

write.table(out_dd_interest_final_main, file = "study1_moderation_interest_main.txt", sep = ",", quote = FALSE, row.names = F, na = "")
write.table(out_dd_interest_final_indi_diff, file = "study1_moderation_interest_individual_and_difference.txt", sep = ",", quote = FALSE, row.names = F, na = "")

write.table(out_dd_partisanship_final_main, file = "study1_moderation_partisanship_main.txt", sep = ",", quote = FALSE, row.names = F, na = "")
write.table(out_dd_partisanship_final_indi_diff, file = "study1_moderation_partisanship_individual_and_difference.txt", sep = ",", quote = FALSE, row.names = F, na = "")

write.table(out_dd_ideology_final_main, file = "study1_moderation_ideology_main.txt", sep = ",", quote = FALSE, row.names = F, na = "")
write.table(out_dd_ideology_final_indi_diff, file = "study1_moderation_ideology_individual_and_difference.txt", sep = ",", quote = FALSE, row.names = F, na = "")

write.table(out_dd_panas_final_main, file = "study1_moderation_happiness_main.txt", sep = ",", quote = FALSE, row.names = F, na = "")
write.table(out_dd_panas_final_indi_diff, file = "study1_moderation_happiness_individual_and_difference.txt", sep = ",", quote = FALSE, row.names = F, na = "")
...
write.table(des_main, file = "study1_descriptives_main_by_condition.txt", sep = ",", quote = FALSE, row.names = F, na = "")
write.table(des_indi_diff, file = "study1_descriptives_individual_and_difference_by_condition.txt", sep = ",", quote = FALSE, row.names = F, na = "")
